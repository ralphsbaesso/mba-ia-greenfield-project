...
// supertest types `body` as `any`; these describe the shapes the API returns so
// assertions are checked against a real contract instead of an untyped bag.
interface ErrorBody {
  error: string;
}
...
describe('Auth (e2e)', () => {
  ...
  async function registerConfirmAndLogin(
    ...
    const res = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ email, password });
    const { access_token, refresh_token } = res.body as TokensBody;
  ...
  describe('POST /auth/register', () => {
    it('returns 201 with { id, email } on valid registration', async () => {
      ...
      const { id, email } = res.body as RegisteredUserBody;
  ...
  describe('JWT Guard', () => {
    ...
    it('returns 200 on GET /auth/me with a valid access token', async () => {
      ...
      const { sub, email } = res.body as CurrentUserBody;
  ...
  describe('POST /auth/refresh', () => {
    it('returns 200 with new access_token and refresh_token on valid refresh token', async () => {
      ...
      const rotated = res.body as TokensBody;
    ...
    it('returns 200 with valid access token when reuse is within grace period', async () => {
      ...
      expect((res.body as TokensBody).access_token).toBeDefined();
  ...
  async function capturePasswordResetToken(email: string): Promise<string> {
    let captured = '';
    jest
      .spyOn(app.get(MailService), 'sendPasswordResetEmail')
      .mockImplementationOnce((_email, _name, token) => {
        captured = token;
        return Promise.resolve();
      });
  ...
  describe('POST /auth/reset-password', () => {
    it('returns 204 with a valid token and updates the password', async () => {
      ...
      const loginRes = await request(app.getHttpServer())
        ...
        .expect(200);
      expect((loginRes.body as TokensBody).access_token).toBeDefined();
    });
